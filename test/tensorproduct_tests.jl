...
@testset "$(rpad("Tensor Product Tests",80))" begin
    @testset "$(rpad("construction: degree, domain and condition are per-axis",76))" begin
        ...
        # `kron` has no one-argument method, so a one-factor product cannot be assembled by
        # splatting into it. Nothing else here builds a quadrature on a `D = 1` basis.
        ...
        # formed on demand, as the docstring says, so not the factor's own storage
        M1 = mass_matrix(op1)
        @test M1 !== mass_matrix(only(mass_factors(op1)))
        M1[1, 1] = 0
        @test mass_matrix(only(mass_factors(op1)))[1, 1] != 0
        @test mass_matrix(op1)[1, 1] != 0
